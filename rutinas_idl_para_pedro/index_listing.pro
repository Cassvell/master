...
;##############################################################################
        ...
        FOR i=0ll, file_number-1 DO BEGIN
                ...
                CALDAT, tmp_julday+i, tmp_month, tmp_day, tmp_year
                string_date[i]    = string(tmp_year, tmp_month, tmp_day, FORMAT='(I4,I02,I02)')
                data_file_name[i] = data_path+'/dH_teo/teo_'+string_date[i]+'.dst.early'
		        file_dh = FILE_SEARCH(data_file_name[i], COUNT=opened_files)   		                     
        ENDFOR
...
;print, H[idx]
    ...
    for i=0, N_ELEMENTS(idx)-1 do begin
           if d_idx_tmp[i] ne 0 then begin
                ...
                print, y_idx_tmp[i], m_idx_tmp[i], d_idx_tmp[i], hr_idx_tmp[i], $
                H_idx_tmp[i], $
                FORMAT = '(I4, "-", I02, "-", I02, 2X, I02, 4X, F6.1)' 
